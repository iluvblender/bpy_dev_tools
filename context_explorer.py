...
class LabelOp(bpy.types.Operator):
    ...
    def draw(self, context):
        ...
        layout.box().label(text=self.data_path)
        layout.label(text=path_type)
...
class ContextExplorer(bpy.types.Operator):
    ...
    def draw(self, context):
        ...
        header.label(text=area.type)
        header.label(text=area.name)
        header.label(text=area.description)
        ...
        def draw_item(layout, item):
            if isinstance(item, bpy.types.ID):
                ...
                layout.label(text=item.object.name)
            else:
                layout.label(text=str(item))
        ...
        for name in sorted(attributes - ignored_attributes):
            ...
            if (not name.startswith('__')) and value:
                split_C_prop = column1.box().split(factor=0.4)
                ...
                if isinstance(value, collections.abc.Sequence):
                    ...
                    split_C_prop_description.label(text=description(value[0]))
                    for item in value:
                        draw_item(column, item)
                else:
                    oper_props.data_path_type = str(value.__class__)
                    split_C_prop_description.label(text=str(value))
